"""Show how derivative gain Kd affects DC motor speed control."""
...
def _print_results(results):
    """Print Kd tuning metrics."""
    print("Kd Tuning for DC Motor Speed Control:")
    print("Derivative action can reduce overshoot and smooth the response.")
    print(
        "Too much Kd can make control action noisy or overly damped, "
        "though this simulation does not include measurement noise."
    )
    print()
    print("Kd      Final Speed   Final Error   Overshoot   Settling Time   Max Voltage")
...
def _draw_plots(simulations, target_speed):
    """Draw speed responses for the Kd sweep."""
    ...
    axes[0].set_title("Effect of Kd on Motor Speed Response")
...
def main():
    """Run a derivative gain tuning comparison."""
    target_speed = 80.0
    Kp = 0.16
    Ki = 0.018
    Kd_values = [0.0, 0.04, 0.12, 0.25]
    cases = [
        {"label": f"Kd = {Kd}", "Kp": Kp, "Ki": Ki, "Kd": Kd}
        for Kd in Kd_values
    ]
