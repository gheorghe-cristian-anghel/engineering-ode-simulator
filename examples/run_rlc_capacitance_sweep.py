"""Run and plot a capacitance sweep for a series RLC step response."""
...
def _print_results(results):
    """Print capacitance sweep metrics."""
    print("RLC Capacitance Sweep:")
    print("Changing C changes the natural frequency and response time scale.")
    print("Larger C usually slows the response because energy storage is larger.")
    print("Damping ratio also changes because zeta depends on C.")
    print()
    print(
        "C (F)     Zeta      Omega_n    Final V    Peak V     "
        "Overshoot   Settling"
    )
    print(
        "          (-)       (rad/s)    (V)        (V)        "
...
def _draw_plot(simulations, vin):
    """Draw capacitor-voltage responses for all capacitance values."""
    plt.figure()

    for simulation in simulations:
        label = f"C = {simulation['parameter_value']:.2f} F"
        plt.plot(simulation["t"], simulation["capacitor_voltage"], label=label)

    plt.axhline(vin, color="gray", linestyle=":", label="Input voltage")
    plt.title("RLC Capacitor Voltage Response: Capacitance Sweep")
    plt.xlabel("Time (s)")
...
def main():
    """Sweep capacitance while keeping resistance and inductance fixed."""
